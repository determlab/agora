...
| D12 | **The transcript renders somebody else's words, so it escapes and builds its markup in ONE pass over the raw text - a second pattern over the first pass's output is where the hole opens** - every piece goes through `esc()` before it is concatenated with a literal tag, and nothing the renderer has produced is ever scanned again. Direction is settled by the same restraint: a message body carries `dir="auto"`, because direction is a property of the message and not of the page, and the Latin runs inside it are left alone. Isolating them was tried and measured in a browser: an isolate is one neutral object to the paragraph around it, so `roadmap.md #141` rendered `#141 roadmap.md` and `@bob later` rendered `later @bob`, while the tokens it was meant to protect resolve correctly without it. The consequence is that a new transcript feature is another branch of that single pass, never a regex over the HTML the last one emitted - and that the page adds no bidi control of its own. **Numbered D12, not D10**: D10 landed first from issue #26 (heartbeat); D11 is issue #31's own row, filed in a still-open branch reconciled at the same time as this one. | issue #25; `withMentions` in `static/index.html`; asserted by `tests/test_ui.py` - `test_the_message_renderer_escapes_every_piece_it_emits`, `test_every_message_body_carries_dir_auto`, `test_a_latin_run_inside_a_mixed_message_is_not_isolated` |
